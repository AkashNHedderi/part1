set ns [new Simulator]

set tf [open prog3.tr w]
$ns trace-all $tf

set nf [open prog3.nam w]
$ns namtrace-all $nf

set s [$ns node]
set c [$ns node]

$ns color 1 Blue

$s label "Server"
$c label "Client"

$ns duplex-link $s $c 10Mb 22ms DropTail 
$ns duplex-link-op $s $c orient right


set tcp0 [new Agent/TCP]
$ns attach-agent $s $tcp0
set tcpsink [new Agent/TCPSink]
$ns attach-agent $c $tcpsink 
$ns connect $tcp0 $tcpsink 
set ftp0 [new Application/FTP]
$ftp0 attach-agent $tcp0 


$ns at 1.0 "$ftp0 start"
$ns at 15 "$ftp0 stop"

$ns at 15.01 "finish"

$tcp0 set fid_ 1

proc finish {} {

	global ns tf nf 
	$ns flush-trace
	close $nf 
	close $tf 
	exec nam prog3.nam &
	exec awk -f ex3transfer.awk prog3.tr &
	exec awk -f ex3convert.awk  prog3.tr > convert.tr &
	exec xgraph convert.tr -geometry 800*400 -t "bytes_received_at_client" -x "time_in_secs" -y "bytes_in"
	exit 0 
}

$ns run 
